...
namespace massivegodotintegration.bepu_physics_test;

public partial class BepuPhysicstest : Godot.Node3D {
	...
	public override void _Ready() {
		...
		_bodyHandle = _simulation.Bodies.Add(
		...
		_staticHandle = _simulation.Statics.Add(
			new StaticDescription(
				new Vector3(0, 0, 0), 
				_simulation.Shapes.Add(new Box(500, 1, 500))
			)
		);
	...
	public override void _PhysicsProcess(double delta) {
		...
		// _simulation.Timestep((float)delta, _threadDispatcher);
		_simulation.Timestep((float)delta, _threadDispatcher);

		//You can use the body handle to look up information about a body; the bodies collection is indexable:
		var bodyReference = _simulation.Bodies[_bodyHandle];
		...
		GD.Print($"Body position at {bodyReference.Pose.Position}");
		
		DebugDraw3D.DrawSphere(new Godot.Vector3(bodyReference.Pose.Position.X, bodyReference.Pose.Position.Y, bodyReference.Pose.Position.Z), 1f, Colors.Red);
		DebugDraw3D.DrawBox(new Godot.Vector3(0, 0, 0), Quaternion.Identity, new Godot.Vector3(500, 1, 500), Colors.Green, true);
		bodyReference.Velocity.Linear += new Vector3(3, 0, 0) * (float)delta;
	}
